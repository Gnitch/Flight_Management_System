...
if(isset($_POST['new-pwd-submit'])) {
    ...
    if($password !== $password_repeat) {
        header('Location: ../views/create-new-pwd.php?err=pwdnotmatch'); 
        exit();
    }
    $curr_date = date('U');
    ...
    if(!mysqli_stmt_prepare($stmt,$sql)) {        
        header('Location: ../views/create-new-pwd.php?err=sqlerr'); 
        exit();            
    } else {
                    ...
                    if(!$row = mysqli_fetch_assoc($result)) {
                        ...
                        if(!mysqli_stmt_prepare($stmt,$sql)) {
                            ...
                            if(!mysqli_stmt_prepare($stmt,$sql)) {
                                ...
                                mysqli_stmt_bind_param($stmt,'s',$token_email);            
                                mysqli_stmt_execute($stmt);
                                header('Location: ../views/login.php?pwd=updated');    
                            }                            
                        }                        
                    }
                }                
    ...
    header('Location: ../views/create-new-pwd.php');    
} 
