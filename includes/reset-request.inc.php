...
if(isset($_POST['reset-req-submit'])) {
    ...
    if(!filter_var($user_email,FILTER_VALIDATE_EMAIL)) {
        header('Location: ../views/reset-pwd.php?err=invalidemail');    
        exit();
    }    
    $sql = 'DELETE FROM PwdReset WHERE pwd_reset_email=?;';
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt,$sql)) { 
        header('Location: ../views/reset-pwd.php?err=sqlerr');    
        exit();            
    } else {
    ...
    try {     
        ...
        $mail->Send();
        header('Location: ../views/reset-pwd.php?mail=success');       
    } 
    catch(Exception $e) {        
        // echo $mail->ErrorInfo;
        header('Location: ../views/reset-pwd.php?err=mailerr');      
    }
