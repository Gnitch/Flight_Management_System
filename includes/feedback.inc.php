...
if(isset($_POST['feed_but'])) {
    ...
    if(!filter_var($email,FILTER_VALIDATE_EMAIL)) {
        header('Location: ../views/feedback.php?error=invalidemail');
        exit();
    }
    $sql = 'INSERT INTO Feedback (email,q1,q2,q3,rate) VALUES (?,?,?,?,?)';
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt,$sql)) {
        header('Location: ../views/feedback.php?error=sqlerror');
        exit();            
    } else {
        mysqli_stmt_bind_param($stmt,'ssssi',$email,$q1,$q2,$q3,$stars);            
        mysqli_stmt_execute($stmt); 
        header('Location: ../views/feedback.php?error=success');
        exit();       
