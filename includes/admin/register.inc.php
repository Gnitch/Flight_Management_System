<?php
if(isset($_POST['signup_submit']) and isset($_SESSION['adminId'])) {    
    ...
    if(!filter_var($email_id,FILTER_VALIDATE_EMAIL)) {
        header('Location: ../../views/admin/register.php?error=invalidemail');
        exit();
    }
    else if($password !== $password_repeat) {
        header('Location: ../../views/admin/register.php?error=pwdnotmatch');
        exit();
    }
    else {
        ...
        if($username_check > 0) {
            header('Location: ../../views/admin/register.php?error=usernameexists');
            exit();
        } else {
            ...
            if($email_check > 0) {
                header('Location: ../../views/admin/register.php?error=emailexists');
                exit();
            } else {
